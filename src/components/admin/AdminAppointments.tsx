...
export const AdminAppointments = () => {
  ...
  const filteredAppointments = appointments.filter(apt => formatDate(apt.date) === format(selectedDate, 'yyyy-MM-dd'));
  ...
  const handleStatusChange = (id: string, status: 'pending' | 'confirmed' | 'cancelled') => {
    ...
    if (status === 'confirmed') {
      ...
      const conflictingAppointment = appointments.find(apt => apt.id !== id && apt.status === 'confirmed' && formatDate(apt.date) === formatDate(appointmentToConfirm.date) && apt.time === appointmentToConfirm.time);
      if (conflictingAppointment) {
        toast.error(`Já existe um agendamento confirmado para ${appointmentToConfirm.time} nesta data`);
        return;
      }
    }
    const updatedAppointments = appointments.map(apt => apt.id === id ? {
      ...apt,
      status
    } : apt);
  ...
  return <div className="grid md:grid-cols-[300px,1fr] gap-6">
      <div className="space-y-4">
        <Card className="p-4">
          <Calendar mode="single" selected={selectedDate} onSelect={date => date && setSelectedDate(date)} className="rounded-md pointer-events-auto" locale={ptBR} />
        </Card>

        <Card className="p-4">
          <h3 className="text-lg font-semibold mb-4">Horários Disponíveis</h3>
          <div className="mb-4 flex space-x-2">
            <Input placeholder="HH:MM" value={newTimeSlot} onChange={e => setNewTimeSlot(e.target.value)} />
          ...
          <div className="space-y-2">
            {timeSlots.sort((a, b) => a.time.localeCompare(b.time)).map(slot => <div key={slot.id} className="flex justify-between items-center p-2 bg-background rounded-md">
      ...
      <div className="space-y-4">
        ...
        {filteredAppointments.length === 0 ? <p className="text-muted-foreground">Nenhum agendamento para esta data.</p> : filteredAppointments.map(appointment => <Card key={appointment.id} className="p-4">
              <div className="flex justify-between items-start">
                ...
                <div className="text-sm space-y-2">
                  <Button size="sm" variant={appointment.status === 'confirmed' ? 'default' : 'outline'} className={appointment.status === 'confirmed' ? 'bg-green-500 hover:bg-green-600 w-full' : 'w-full'} onClick={() => handleStatusChange(appointment.id, 'confirmed')}>Confirmado</Button>
                  <Button size="sm" variant={appointment.status === 'pending' ? 'default' : 'outline'} className={appointment.status === 'pending' ? 'bg-yellow-500 hover:bg-yellow-600 w-full' : 'w-full'} onClick={() => handleStatusChange(appointment.id, 'pending')}>
                    Pendente
                  </Button>
                  <Button size="sm" variant={appointment.status === 'cancelled' ? 'default' : 'outline'} className={appointment.status === 'cancelled' ? 'bg-red-500 hover:bg-red-600 w-full' : 'w-full'} onClick={() => handleStatusChange(appointment.id, 'cancelled')}>
                    Cancelar
                  </Button>
                </div>
              </div>
